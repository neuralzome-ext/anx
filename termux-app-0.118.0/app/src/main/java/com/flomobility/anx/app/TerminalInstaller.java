...
import static com.flomobility.anx.shared.terminal.TerminalConstants.TERMUX_PREFIX_DIR;
import static com.flomobility.anx.shared.terminal.TerminalConstants.TERMINAL_PREFIX_DIR_PATH;
import static com.flomobility.anx.shared.terminal.TerminalConstants.TERMUX_STAGING_PREFIX_DIR;
...
public class TerminalInstaller {
    ...
    public static void setupBootstrapIfNeeded(final Activity activity, final Runnable whenDone) {
        ...
        if (FileUtils.directoryFileExists(TERMINAL_PREFIX_DIR_PATH, true)) {
  ...
  //      final ProgressDialog progress = ProgressDialog.show(activity, null, activity.getString(R.string.bootstrap_installer_body), true, false);
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    Logger.logInfo(LOG_TAG, "Installing " + TerminalConstants.TERMINAL_APP_NAME + " bootstrap packages.");
                    ...
                    error = TerminalFileUtils.isTermuxPrefixStagingDirectoryAccessible(true, true);
                    if (error != null) {
                        showBootstrapErrorDialog(activity, whenDone, Error.getErrorMarkdownString(error));
                        return;
                    }
                    ...
                    error = TerminalFileUtils.isTermuxPrefixDirectoryAccessible(true, true);
                    if (error != null) {
                        showBootstrapErrorDialog(activity, whenDone, Error.getErrorMarkdownString(error));
                        return;
                    }
    ...
    static void setupStorageSymlinks(final Context context) {
        ...
        new Thread() {
            public void run() {
                try {
                    Error error;
                    File storageDir = TerminalConstants.TERMUX_STORAGE_HOME_DIR;
