...
uvc_frame_t *
UVCPreview::draw_preview_one(uvc_frame_t *frame, ANativeWindow **window, convFunc_t convert_func,
                             int pixcelBytes) {
    // ENTER();
...
void *UVCPreview::capture_thread_func(void *vptr_args) {
    int result;
    LOGI("JAIMIN: Inside capture frame thread");
    ENTER();
    UVCPreview *preview = reinterpret_cast<UVCPreview *>(vptr_args);
    if (LIKELY(preview)) {
        JavaVM *vm = getVM();
        ...
        LOGI("JAIMIN: Inside capture frame thread 2");
        // detach from JavaVM
        vm->DetachCurrentThread();
        MARK("DetachCurrentThread");
    }
