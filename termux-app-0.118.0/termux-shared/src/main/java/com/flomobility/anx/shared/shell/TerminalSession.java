 ...
 * It also provides a way to link each {@link com.flomobility.anx.terminal.TerminalSession} with the {@link ExecutionCommand}
 * that started it.
 */
public class TerminalSession {

    private final com.flomobility.anx.terminal.TerminalSession mTerminalSession;
     ...
     * @param sessionName The optional {@link com.flomobility.anx.terminal.TerminalSession} name.
     * @param setStdoutOnExit If set to {@code true}, then the {@link ResultData#stdout}
     *                        available in the {@link TermuxSessionClient#onTermuxSessionExited(TerminalSession)}
     *                        callback will be set to the {@link com.flomobility.anx.terminal.TerminalSession} transcript. The session
     ...
     * @return Returns the {@link TerminalSession}. This will be {@code null} if failed to start the execution command.
     */
    public static TerminalSession execute(@NonNull final Context context, @NonNull ExecutionCommand executionCommand,
        ...
        if (!executionCommand.setState(ExecutionCommand.ExecutionState.EXECUTING)) {
            executionCommand.setStateFailed(Errno.ERRNO_FAILED.getCode(), context.getString(R.string.error_failed_to_execute_terminal_session_command, executionCommand.getCommandIdAndLabelLogString()));
            TerminalSession.processTermuxSessionResult(null, executionCommand);
            return null;
        }
        ...
        com.flomobility.anx.terminal.TerminalSession terminalSession = new com.flomobility.anx.terminal.TerminalSession(executionCommand.executable, executionCommand.workingDirectory, executionCommand.arguments, environment, executionCommand.terminalTranscriptRows, terminalSessionClient);

        if (sessionName != null) {
            terminalSession.mSessionName = sessionName;
        }

        return new TerminalSession(terminalSession, executionCommand, termuxSessionClient, setStdoutOnExit);
    }

    /**
     * Signal that this {@link TerminalSession} has finished.  This should be called when
     * {@link TerminalSessionClient#onSessionFinished(com.flomobility.anx.terminal.TerminalSession)} callback is received by the caller.
     *
     ...
     * and then calls {@link #processTermuxSessionResult(TerminalSession, ExecutionCommand)} to process the result}.
     *
     */
    public void finish() {
        ...
        TerminalSession.processTermuxSessionResult(this, null);
    }

    /**
     * Kill this {@link TerminalSession} by sending a {@link OsConstants#SIGILL} to its {@link #mTerminalSession}
     * if its still executing.
     *
     * @param context The {@link Context} for operations.
     * @param processResult If set to {@code true}, then the {@link #processTermuxSessionResult(TerminalSession, ExecutionCommand)}
    ...
    /**
     * Process the results of {@link TerminalSession} or {@link ExecutionCommand}.
     *
     ...
     * then the {@link TerminalSession.TermuxSessionClient#onTermuxSessionExited(TerminalSession)}
     * callback will be called.
     *
     * @param terminalSession The {@link TerminalSession}, which should be set if
     ...
     */
    private static void processTermuxSessionResult(final TerminalSession terminalSession, ExecutionCommand executionCommand) {
        if (terminalSession != null)
            executionCommand = terminalSession.mExecutionCommand;
        ...
        if (terminalSession != null && terminalSession.mTermuxSessionClient != null) {
            terminalSession.mTermuxSessionClient.onTermuxSessionExited(terminalSession);
        } else {
    ...
    public com.flomobility.anx.terminal.TerminalSession getTerminalSession() {
    ...
    public interface TermuxSessionClient {
        ...
        /**
         * Callback function for when {@link TerminalSession} exits.
         *
         * @param terminalSession The {@link TerminalSession} that exited.
         */
        void onTermuxSessionExited(TerminalSession terminalSession);
