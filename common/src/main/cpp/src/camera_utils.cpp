...
const char* GetPairStr(T key, std::vector<std::pair<T, const char*>>& store) {
    ...
    LOGW(TAG, "(%#08x) : UNKNOWN_TAG for %s", key, typeid(store[0].first).name());
    return UKNOWN_TAG;
}
...
static std::vector<FORMAT_PAIR> formatInfo{
        ...
        MAKE_PAIR(AIMAGE_FORMAT_RGB_888),
        MAKE_PAIR(AIMAGE_FORMAT_RGBA_8888),
};
...
#define MODE_LABLE "ModeInfo:"
    ...
    ASSERT(TAG, !(val.count & 0x3), "STREAM_CONFIGURATION (%d) should multiple of 4",
           val.count);
    ASSERT(TAG, val.type == ACAMERA_TYPE_INT32,
...
void PrintCameras(ACameraManager* cmrMgr) {
    ...
    LOGI(TAG, "camera Status = %d, %s", status, GetErrorStr(status));

    for (int i = 0; i < cameraIds->numCameras; ++i) {
        const char* id = cameraIds->cameraIds[i];
        LOGI(TAG, "=====cameraId = %d, cameraName = %s=====", i, id);
...
void PrintRequestMetadata(ACaptureRequest* req) {
    ...
    for (int32_t idx = 0; idx < count; ++idx) {
        ...
        for (uint32_t i = 0; i < val.count; ++i) {
            switch (val.type) {
                case ACAMERA_TYPE_INT32:
                    LOGI(TAG, "Capture Tag %s: %08d", name, val.data.i32[i]);
                    break;
                case ACAMERA_TYPE_BYTE:
                    LOGI(TAG, "Capture Tag %s: %#08x", name, val.data.u8[i]);
                    break;
                case ACAMERA_TYPE_INT64:
                    LOGI(TAG, "Capture Tag %s: %" PRIu64, name, (int64_t)val.data.i64[i]);
                    break;
                case ACAMERA_TYPE_FLOAT:
                    LOGI(TAG, "Capture Tag %s: %f", name, val.data.f[i]);
                    break;
                case ACAMERA_TYPE_DOUBLE:
                    LOGI(TAG, "Capture Tag %s: %" PRIx64, name, val.data.i64[i]);
                    break;
                case ACAMERA_TYPE_RATIONAL:
                    LOGI(TAG, "Capture Tag %s: %08x, %08x", name, val.data.r[i].numerator,
                         val.data.r[i].denominator);
                    break;
                default:
                    ASSERT(TAG, false, "Unknown tag value type: %d", val.type);
            }
        }
    }
}
...
void PrintCameraDeviceError(int err) {
    LOGI(TAG, "CameraDeviceError(%#x): %s", err, GetCameraDeviceErrorStr(err));
}
