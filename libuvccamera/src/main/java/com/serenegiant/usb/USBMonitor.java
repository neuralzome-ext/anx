...
public final class USBMonitor {
    ...
    private static final String ACTION_USB_PERMISSION_BASE = "com.flomobility.USB_PERMISSION.";
     ...
     */
    private final ConcurrentHashMap<String, UsbControlBlock> mCtrlBlocks = new ConcurrentHashMap<String, UsbControlBlock>();
    ...
    public void destroy() {
        ...
        if (!destroyed) {
            ...
            final Set<String> keys = mCtrlBlocks.keySet();
            if (keys != null) {
    ...
    public UsbControlBlock openDevice(final UsbDevice device) throws SecurityException {
        if (hasPermission(device)) {
            ...
            if (result == null) {
                ...
                mCtrlBlocks.put(device.getDeviceName(), result);
            }
            return result;
        } else {
    ...
    public final void processConnect(final UsbDevice device) {
        ...
        mAsyncHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                ctrlBlock = mCtrlBlocks.get(device.getDeviceName());
                if (ctrlBlock == null) {
                    ctrlBlock = new UsbControlBlock(USBMonitor.this, device);
                    mCtrlBlocks.put(device.getDeviceName(), ctrlBlock);
                    createNew = true;
                } else {
                    createNew = false;
                }
