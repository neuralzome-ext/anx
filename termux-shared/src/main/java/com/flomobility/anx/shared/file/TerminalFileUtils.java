package com.flomobility.anx.shared.file;
...
import com.flomobility.anx.shared.models.errors.FileUtilsErrno;
...
public class TerminalFileUtils {
    ...
    public static List<String> getExpandedTermuxPaths(List<String> paths) {
        ...
        for (int i = 0; i < paths.size(); i++) {
            expandedPaths.add(getExpandedTerminalPath(paths.get(i)));
        }
     ...
     */
    public static String getExpandedTerminalPath(String path) {
        if (path != null && !path.isEmpty()) {
            path = path.replaceAll("^\\$PREFIX$", TerminalConstants.TERMINAL_PREFIX_DIR_PATH);
            path = path.replaceAll("^\\$PREFIX/", TerminalConstants.TERMINAL_PREFIX_DIR_PATH + "/");
            path = path.replaceAll("^~/$", TerminalConstants.TERMINAL_HOME_DIR_PATH);
            path = path.replaceAll("^~/", TerminalConstants.TERMINAL_HOME_DIR_PATH + "/");
        }
     ...
     */
    public static List<String> getUnExpandedTerminalPaths(List<String> paths) {
        if (paths == null) return null;
        List<String> unExpandedPaths = new ArrayList<>();

        for (int i = 0; i < paths.size(); i++) {
            unExpandedPaths.add(getUnExpandedTerminalPath(paths.get(i)));
        }
     ...
     */
    public static String getUnExpandedTerminalPath(String path) {
        if (path != null && !path.isEmpty()) {
            path = path.replaceAll("^" + Pattern.quote(TerminalConstants.TERMINAL_PREFIX_DIR_PATH) + "/", "\\$PREFIX/");
            path = path.replaceAll("^" + Pattern.quote(TerminalConstants.TERMINAL_HOME_DIR_PATH) + "/", "~/");
        }
     ...
     *                   {@link TerminalFileUtils#getExpandedTerminalPath(String)} before its passed to
    ...
    public static String getCanonicalPath(String path, final String prefixForNonAbsolutePath, final boolean expandPath) {
        ...
        if (expandPath)
            path = getExpandedTerminalPath(path);
     ...
     * @return Returns the allowed path if it {@code path} is under it, otherwise {@link TerminalConstants#TERMUX_FILES_DIR_PATH}.
     */
    public static String getMatchedAllowedTermuxWorkingDirectoryParentPathForPath(String path) {
        if (path == null || path.isEmpty()) return TerminalConstants.TERMUX_FILES_DIR_PATH;

        if (path.startsWith(TerminalConstants.TERMUX_STORAGE_HOME_DIR_PATH + "/")) {
            return TerminalConstants.TERMUX_STORAGE_HOME_DIR_PATH;
        } if (path.startsWith(Environment.getExternalStorageDirectory().getAbsolutePath() + "/")) {
            return Environment.getExternalStorageDirectory().getAbsolutePath();
        } else if (path.startsWith("/sdcard/")) {
            return "/sdcard";
        } else {
            return TerminalConstants.TERMUX_FILES_DIR_PATH;
        }
    }
    ...
    public static Error validateDirectoryFileExistenceAndPermissions(String label, final String filePath, final boolean createDirectoryIfMissing,
        ...
        return FileUtils.validateDirectoryFileExistenceAndPermissions(label, filePath,
            TerminalFileUtils.getMatchedAllowedTermuxWorkingDirectoryParentPathForPath(filePath), createDirectoryIfMissing,
    ...
    /**
     * Validate if {@link TerminalConstants#TERMUX_FILES_DIR_PATH} exists and has
     ...
     * {@link TerminalConstants#TERMINAL_PREFIX_DIR_PATH} and the path must be accessible.
     *
     * The permissions set to directory will be {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS}.
     *
     ...
     * defined by default by {@link TerminalConstants#TERMUX_FILES_DIR_PATH} where id will be 0 for
     * primary user and a higher number for other users/profiles. If app is running under work profile
     * or secondary user, then {@link TerminalConstants#TERMUX_FILES_DIR_PATH} will not be accessible
    ...
    public static Error isTermuxFilesDirectoryAccessible(@NonNull final Context context, boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        ...
        if (!FileUtils.directoryFileExists(TerminalConstants.TERMUX_FILES_DIR_PATH, true))
            return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError("termux files directory", TerminalConstants.TERMUX_FILES_DIR_PATH);

        if (setMissingPermissions)
            FileUtils.setMissingFilePermissions("termux files directory", TerminalConstants.TERMUX_FILES_DIR_PATH,
                FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS);

        return FileUtils.checkMissingFilePermissions("termux files directory", TerminalConstants.TERMUX_FILES_DIR_PATH,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, false);
    }

    /**
     * Validate if {@link TerminalConstants#TERMINAL_PREFIX_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     * .
     *
     * The {@link TerminalConstants#TERMINAL_PREFIX_DIR_PATH} directory would not exist if termux has
    ...
    public static Error isTermuxPrefixDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
           return FileUtils.validateDirectoryFileExistenceAndPermissions("termux prefix directory", TerminalConstants.TERMINAL_PREFIX_DIR_PATH,
    ...
    /**
     * Validate if {@link TerminalConstants#TERMUX_STAGING_PREFIX_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     *
     * @param createDirectoryIfMissing The {@code boolean} that decides if directory file
     *                                 should be created if its missing.
    ...
    public static Error isTermuxPrefixStagingDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        return FileUtils.validateDirectoryFileExistenceAndPermissions("termux prefix staging directory", TerminalConstants.TERMUX_STAGING_PREFIX_DIR_PATH,
            null, createDirectoryIfMissing,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, setMissingPermissions, true,
            false, false);
    }
    ...
    public static String getTermuxFilesStatMarkdownString(@NonNull final Context context) {
        ...
        statScript
            ...
            .append(" '/data/data'")
            .append(" '/data/user/0'")
            .append(" '" + TerminalConstants.TERMUX_INTERNAL_PRIVATE_APP_DATA_DIR_PATH + "'")
            .append(" '/data/user/0/" + TerminalConstants.TERMINAL_PACKAGE_NAME + "'")
            .append(" '" + TerminalConstants.TERMUX_FILES_DIR_PATH + "'")
            .append(" '" + filesDir + "'")
            .append(" '/data/user/0/" + TerminalConstants.TERMINAL_PACKAGE_NAME + "/files'")
            .append(" '/data/user/" + TerminalConstants.TERMINAL_PACKAGE_NAME + "/files'")
            .append(" '" + TerminalConstants.TERMUX_STAGING_PREFIX_DIR_PATH + "'")
            .append(" '" + TerminalConstants.TERMINAL_PREFIX_DIR_PATH + "'")
            .append(" '" + TerminalConstants.TERMINAL_HOME_DIR_PATH + "'")
            .append(" '" + TerminalConstants.TERMINAL_BIN_PREFIX_DIR_PATH + "/login'")
            .append(" 2>&1")
        ...
        executionCommand.commandLabel = TerminalConstants.TERMINAL_APP_NAME + " Files Stat Command";
        ...
        markdownString.append("## ").append(TerminalConstants.TERMINAL_APP_NAME).append(" Files Info\n\n");
        AndroidUtils.appendPropertyToMarkdown(markdownString,"TERMUX_REQUIRED_FILES_DIR_PATH ($PREFIX)", TerminalConstants.TERMUX_FILES_DIR_PATH);
